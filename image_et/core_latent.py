...
##################################################################
# Edited ET class with 3 blocks:
#  - 1st block has 1 iteration
#  - 2nd block has 10 iterations
#  - 3rd block has 1 iteration
##################################################################
class ET(nn.Module):
    def __init__(self, 
                 x: TENSOR, 
        ...
        # We define THREE (norm, ETBlock) pairs:
        ...
        # Corresponding time steps for each block
        self.timesteps = [1, 10, 1]
    ...
    def forward(self, x: TENSOR, alpha: float = 1.0):
        ...
        # For each (norm, et) pair, run for the specified number of time steps
        for (norm, et), steps in zip(self.blocks, self.timesteps):
            for _ in range(steps):
                g = norm(x)
                dEdg, E = torch.func.grad_and_value(et)(g)
                x = x - alpha * dEdg
        
        # Final classification token
        x = self.decode(x[:, 0])  # Use CLS token for classification
        return x
