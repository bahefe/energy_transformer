...
def my_logsumexp(x: TENSOR, dim: int) -> TENSOR:
    # Find the maximum value along the given dimension for numerical stability.
    m, _ = x.max(dim=dim, keepdim=True)
    # Subtract the max, exponentiate, sum, take log, then add back the max.
...
class Attention(nn.Module):
    ...
    def forward(self, g: TENSOR, mask: TENSOR = None):
        ...
        if mask is not None:
            A *= mask
        # Use our manual log-sum-exp function instead of torch.logsumexp.
        energy = (-1.0 / self.beta) * my_logsumexp(self.beta * A, dim=-1).sum()
        return energy
    ...
    def forward(self, x: TENSOR, alpha: float = 1.0):
        ...
        for norm, et in self.blocks:
            for _ in range(self.K):
                g = norm(x)
                # Compute gradient of energy w.r.t. g and update x
                dEdg, E = torch.func.grad_and_value(et)(g)
                x = x - alpha * dEdg
        
        x = self.decode(x[:, 0])  # Use the CLS token for classification.
        return x
